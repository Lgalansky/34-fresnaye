const CACHE='fresnaye-v96-portable-r4';
...
self.addEventListener('fetch',e=>{
  ...
  e.respondWith((async()=>{const c=await caches.open(CACHE);
    if(SHELL_URLS.has(u)){try{const r=await fetch(e.request,{cache:'no-cache'});if(r.ok)await c.put(u,r.clone());return r}catch(err){const hit=await c.match(u);if(hit)return hit;throw err}}

...
def ver(h):
    a=str(h.get('artifact','')); v=a.rsplit('_v',1)[-1] if '_v' in a else ''
    if not v.isdigit(): raise ValueError('invalid Source Architect artifact/version')
    return 'v'+v
...
def plan(h,hsha,p,psha,c=None):
    ...
    base={'artifact':f'34_Fresnaye_GeometryDeltaPlan_Source_{v}_v1','planner':'34_Fresnaye_GeometryDeltaPlanner_v6','sourceArchitect':v,'sourceHandoffSha256':hsha,'level02EffectSchemaPath':ep,'pipeline':p.get('artifact'),'pipelineSha256':psha,'canonicalParent':canonical,'viewerReuseCandidate':viewer,'geometryAuthorization':auth,'level02Effect':e,'geometryChanged':False}
    if not auth:
